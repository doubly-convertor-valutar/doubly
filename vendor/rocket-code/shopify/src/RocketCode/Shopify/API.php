...
class API extends Shopify
{
	...
	public function verifyRequest($data = NULL, $bypassTimeCheck = FALSE)
	{
		...
		{
			$urlSegments = explode('.', $da['shop']);
			if (count($urlSegments)!=3 || !(preg_match( "/^[A-Za-z0-9-]+$/", $urlSegments[0]) && $urlSegments[1]=='myshopify' && $urlSegments[2]=='com')) {
				\Log::error('Shop verification failed.');
				throw new \Exception('Shop verification failed.');
			}
		}

		//verify state
		if (array_key_exists('state', $da))
		{
			if ($da['state'] != Cache::get('state_'.$this->_API['SHOP_DOMAIN']))
			{
	...
	{
		//CUSTOM CODE
		$state = Crypt::encrypt('daily_deal'.time());
		//replace special characters as shopify requested
		$processedState = str_replace('=', '%3D', str_replace('%', '%25', str_replace('&', '%26', $state)));
		Cache::put('state_'.$this->_API['SHOP_DOMAIN'], $processedState, 1);
		return 'https://' . $this->_API['SHOP_DOMAIN'] . '/admin/oauth/authorize?client_id=' . $this->_API['API_KEY'] . '&scope=' . implode(',', $data['permissions']) . (!empty($data['redirect']) ? '&redirect_uri=' . urlencode($data['redirect']) : '') . '&state=' . urlencode($processedState);
		//CUSTOM CODE END
	}
	...
	public function setup($data = array())
	{
		...
		//CUSTOM CODE
		$this->_API['API_KEY'] = env('APP_CLIENT_ID', '');
		$this->_API['API_SECRET'] = env('APP_CLIENT_SECRET', '');
		//CUSTOM CODE END
	}
	 ...
	 * @throws \Exception
	 */
    public function call($userData = array(), $verifyData = TRUE, $exclude404Errors = FALSE)
    {
	    if ($verifyData)
	    {
		    foreach (self::$_KEYS as $k)
	    ...
	    if ($_ERROR['NUMBER'])
	    {
	    	if (!$exclude404Errors || ($exclude404Errors && strpos($_ERROR['MESSAGE'], '404 Not Found') === false && strpos($_ERROR['MESSAGE'], '401') === false)) {
